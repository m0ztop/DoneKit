...
const getViewFromPath = (): 'support' | 'privacy' | 'terms' => {
...
const App: React.FC = () => {
  const [view, setView] = useState<'support' | 'privacy' | 'terms'>(getViewFromPath);

  // Update view when browser back/forward is used
  useEffect(() => {
  ...
  const navigateTo = (newView: 'support' | 'privacy' | 'terms') => {
    const newPath = newView === 'support' ? '/' : `/${newView}`;
